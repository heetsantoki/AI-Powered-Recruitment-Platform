...
# Step wrappers
content = re.sub(
    r"<div style={{ display: 'flex', flexDirection: 'column', gap: (\d+), animation: 'fadeIn 0.4s ease' }}>",
    r"<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }} style={{ display: 'flex', flexDirection: 'column', gap: \1 }}>",
    content
)
...
content = content.replace(
    ...
    "<motion.button key={s} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} type=\"button\" className=\"skill-chip\" onClick={() => addSuggested(s)} style={{ cursor: 'pointer' }}>"
)
content = content.replace(
    "+ {s}\n                </button>",
    "+ {s}\n                </motion.button>"
